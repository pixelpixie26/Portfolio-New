const dropList = document.querySelectorAll(".dropList input");
const fromCurrency = document.querySelector(".From input");
const toCurrency = document.querySelector(".To input");
...
for (let index = 0; index < dropList.length; index++) {
  ...
  dropList[index].addEventListener('keyup',event=>{
      loadFlags(event.target); //calling loadFlag with passing target element as an argument 
  });
}
